...
#            self.labels = self.labels[:26]
    ...
    def save(self, savepath, matfile='no', csv = 'yes', npy = 'no'):
        '''
        ...
        if csv == 'yes':
            ...
            """
            CHANGE THIS SO THE NAMING ACTUALLY WORKS 
            <-------------------------------------------------------------------------->
            """
            ...
            for i in range(self.data.shape[0]):
                if len(self.data.shape) == 3:
                    ...
                    df.to_csv(csvpath + str(outname) + str(cond) +  '.csv',sep=',',header = list(self.labels),compression = None)
                                    # str((self.trl[i,0]/self.Fs)*1000) 
                else:
                    print("there")
                     # lets see what this value is 
                    # for some reason, we always enter this else... when is self.data.shape == 3? 
                    df = pd.DataFrame(self.data[:,:].T)

                    # why str(0) ? 
                    df.to_csv(csvpath + str(outname) +'.csv',sep=',',header = list(self.labels),compression = None)
                                          #str(0)
